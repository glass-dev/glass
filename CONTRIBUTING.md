...
# run tests using numpy
uv run nox -s tests-3.13
ARRAY_BACKEND=numpy uv run nox -s tests-3.13
# run tests using array_api_strict
ARRAY_BACKEND=array_api_strict uv run nox -s tests-3.13
# run tests using jax
ARRAY_BACKEND=jax uv run nox -s tests-3.13
# run tests using every supported array library
ARRAY_BACKEND=all uv run nox -s tests-3.13
```
