...
def test_vmap_galactic_ecliptic(xp: ModuleType) -> None:
    ...
    xpx_testing.assert_equal(vmap, xp.zeros_like(vmap))
...
def test_gaussian_nz(
    ...
    xpx_testing.assert_equal(nz, xp.zeros_like(nz))
...
def test_smail_nz(xp: ModuleType) -> None:
    ...
    xpx_testing.assert_equal(pz, xp.zeros_like(pz))
...
def test_fixed_zbins_default_xp() -> None:
    ...
    xpx_testing.assert_close(np.asarray(zbins), expected_zbins, rtol=1e-15)
...
def test_fixed_zbins_xp_provided(xp: ModuleType) -> None:
    ...
    assert len(zbins) == nbins
    xpx_testing.assert_close(xp.asarray(zbins), expected_zbins, rtol=1e-15)
...
def test_tomo_nz_gausserr(xp: ModuleType) -> None:
    ...
    xpx_testing.assert_equal(binned_nz, xp.zeros_like(binned_nz))
