...
def test_linear_bias(
    ...
    xp_assert_close(glass.linear_bias(delta, b), xp.zeros((2, 2)))
    ...
    xp_assert_close(glass.linear_bias(delta, b), b * delta)
...
def test_loglinear_bias(
    ...
    xp_assert_close(glass.loglinear_bias(delta, b), xp.zeros((2, 2)))
...
def test_broadcast_inputs(xp: ModuleType) -> None:
    ...
    xp_assert_equal(delta, xp.zeros_like(delta))
    assert ngal.shape == dims  # ty: ignore[unresolved-attribute]
    xp_assert_equal(ngal[0, :], ngal_in)  # ty: ignore[not-subscriptable]
    assert vis.shape == delta.shape  # ty: ignore[unresolved-attribute]
    xp_assert_equal(vis[0, 0, :], vis_in)  # ty: ignore[not-subscriptable]
...
def test_compute_density_contrast(
    ...
    xp_assert_equal(n, xp.zeros_like(n))
...
def test_position_weights(
    ...
    for bshape in None, (), (100,), (100, 1):
        for cshape in (100,), (100, 50), (100, 3, 2):
            ...
            xp_assert_close(weights, expected)
...
def test_displace_arg_complex(xp: ModuleType) -> None:
    ...
    xp_assert_close(xp.stack([lon, lat]), xp.asarray([0.0, d]))
...
def test_displace_abs(
    ...
    xp_assert_close(cos_a, xp.cos(abs_alpha))
...
def test_displacement_consistent(
    ...
    xp_assert_close(alpha_out, alpha_in, atol=0.0, rtol=1e-10)
...
def test_displacement_random(
    ...
    xp_assert_close(rot @ xp.asarray([0.0, 0.0, 1.0]), u)

    # meta-check that recovering theta and phi from vector works
    xp_assert_close(xp.atan2(xp.hypot(u[:, 0], u[:, 1]), u[:, 2]), theta)
    xp_assert_close(xp.atan2(u[:, 1], u[:, 0]), phi)
