...
def test_effective_bias(
    ...
    bz = xp.zeros((10,))
    xpx_testing.assert_close(glass.effective_bias(z, bz, w), xp.asarray(0.0))
    ...
    xpx_testing.assert_close(glass.effective_bias(z, bz, w), xp.asarray(0.25))
...
def test_linear_bias(
    ...
    xpx_testing.assert_close(glass.linear_bias(delta, b), xp.zeros((2, 2)))
    ...
    xpx_testing.assert_close(glass.linear_bias(delta, b), b * delta)
...
def test_loglinear_bias(
    ...
    xpx_testing.assert_close(glass.loglinear_bias(delta, b), xp.zeros((2, 2)))
...
def test_broadcast_inputs(xp: ModuleType) -> None:
    ...
    xpx_testing.assert_equal(delta, xp.zeros_like(delta))
    assert ngal.shape == dims  # ty: ignore[unresolved-attribute]
    xpx_testing.assert_equal(ngal[0, :], ngal_in)  # ty: ignore[not-subscriptable]
    assert vis.shape == delta.shape  # ty: ignore[unresolved-attribute]
    xpx_testing.assert_equal(vis[0, 0, :], vis_in)  # ty: ignore[not-subscriptable]
...
def test_compute_density_contrast(
    ...
    xpx_testing.assert_equal(n, xp.zeros_like(n))
...
def test_apply_visibility(xp: ModuleType) -> None:
    ...
    xpx_testing.assert_equal(n[:6], xp.zeros_like(n[:6]))
    xpx_testing.assert_equal(n[6:], n_in[6:])
...
def test_sample_number_galaxies(xp: ModuleType) -> None:
    ...
    xpx_testing.assert_equal(n[:6], xp.zeros_like(n[:6], dtype=xp.int64))
    xpx_testing.assert_close(xp.asarray(n[6:], dtype=xp.float64), n_in[6:], atol=250)
...
def test_positions_from_delta(  # noqa: PLR0915
    ...
    assert int(count) == count
    xpx_testing.assert_close(lon, xp.asarray([]))
    xpx_testing.assert_close(lat, xp.asarray([]))
...
def test_position_weights(
    ...
    for bshape in None, (), (100,), (100, 1):
        for cshape in (100,), (100, 50), (100, 3, 2):
            ...
            xpx_testing.assert_close(weights, expected)
...
def test_displace_arg_complex(xp: ModuleType) -> None:
    ...
    xpx_testing.assert_close(xp.stack([lon, lat]), xp.asarray([0.0, d]))
...
def test_displace_abs(
    ...
    xpx_testing.assert_close(cos_a, xp.cos(abs_alpha))
...
def test_displacement(
    ...
    for from_lon, from_lat, to_lon, to_lat, alpha in data:
        ...
        xpx_testing.assert_close(alpha_, alpha)
...
def test_displacement_consistent(
    ...
    xpx_testing.assert_close(alpha_out, alpha_in, atol=0.0, rtol=1e-10)
...
def test_displacement_random(
    ...
    xpx_testing.assert_close(rot @ xp.asarray([0.0, 0.0, 1.0]), u)

    # meta-check that recovering theta and phi from vector works
    xpx_testing.assert_close(xp.atan2(xp.hypot(u[:, 0], u[:, 1]), u[:, 2]), theta)
    xpx_testing.assert_close(xp.atan2(u[:, 1], u[:, 0]), phi)
