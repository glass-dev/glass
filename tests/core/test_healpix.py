...
def test_alm2map_individual(  # noqa: PLR0913
    ...
    compare.assert_array_equal(xp.asarray(old), new)
...
def test_alm2map_spin(
    ...
    for i in range(len(old)):
        compare.assert_array_equal(xp.asarray(old[i]), new[i])
...
def test_pixwin(
    ...
    assert len(old) == len(new)
    for i in range(len(old)):
        compare.assert_array_equal(xp.asarray(old[i], dtype=xp.float64), new[i])
