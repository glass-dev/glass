...
def test_alm2map_individual(
    ...
    xpx_testing.assert_equal(xp.asarray(old), new)
...
def test_alm2map_spin(
    ...
    for i in range(len(old)):
        xpx_testing.assert_equal(xp.asarray(old[i]), new[i])
...
def test_pixwin(
    ...
    assert len(old) == len(new)
    for i in range(len(old)):
        xpx_testing.assert_equal(xp.asarray(old[i], dtype=xp.float64), new[i])
