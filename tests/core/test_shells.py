...
def test_linear_windows(
    ...
    compare.assert_allclose(xp.stack([w.zeff for w in ws]), zgrid[1:-1])
...
def test_combine(
    ...
    # Check sum of result
    compare.assert_allclose(xp.sum(result), xp.asarray(929.267284))

    # Check integral w.r.t z has not changed
    compare.assert_allclose(uxpx.trapezoid(result, z), xp.asarray(4.643139), rtol=1e-6)
...
def test_radial_window_zeff_none(
    ...
    compare.assert_allclose(w.zeff, xp.asarray(1.0))
