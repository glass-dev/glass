...
def tophat_windows(
    zbins: FloatArray,
    dz: float = 1e-3,
    weight: Callable[[FloatArray], FloatArray] | MISSING = MISSING,
) -> list[RadialWindow]:
    """
    ...
    wht = xp.ones_like if weight is MISSING else weight
    ws = []
...
def _uniform_grid(
    ...
    stop: float,
    *,
    step: float | MISSING = MISSING,
    num: int | MISSING = MISSING,
    ...
    if step is not MISSING and num is MISSING:
        return xp.arange(start, stop + step, step)
    if step is MISSING and num is not MISSING:
