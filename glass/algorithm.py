...
def nnls(
    ...
    *,
    tol: float = 0.0,
    maxiter: int | MISSING | None = MISSING,
) -> FloatArray:
    """
    ...
    if maxiter is MISSING or maxiter is None:
        maxiter = 3 * n
...
def cov_clip(
    cov: FloatArray,
    rtol: float | MISSING | None = MISSING,
) -> FloatArray:
    """
    ...
    if rtol is MISSING or rtol is None:
...
def nearcorr(
    a: FloatArray,
    *,
    tol: float | MISSING | None = MISSING,
    niter: int = 100,
) -> FloatArray:
    """
    ...
    *dim, m, n = a.shape
    ...
    # default tolerance
    if tol is MISSING or tol is None:
