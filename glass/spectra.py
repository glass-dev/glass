...
def make_shell_interpolator(shell_z, 
                            shell_weights,
                            cosmo
    ...
    shell_interp = [UnivariateSpline(chi, shell*dzdchi, k=2, s=0) for shell in shell_weights]
    return shell_interp
...
def make_glass_shell_interpolator(ws, cosmo) -> list[UnivariateSpline]:
    ...
    """
    return [UnivariateSpline(cosmo.comoving_distance(ws[i].za) * (1+ws[i].za), ws[i].wa, k=2, s=0, ext=1) for i in range(len(ws))]
    
def get_cls(num_shells, 
    ...
    for idx_shell in (tqdm(np.arange(num_shells))):
        ...
        pk_nl_new[:,:] = np.sqrt(pk_interpolator(k_int, chi_int)).T*(shells_interpolate[idx_shell](chi_int))[:, None]
        ...
        for i_ell, val_ell in enumerate(ell):
            if load_levin is True:
                ...
                lev = levin.pylevin(integral_type, chi_int, pk_nl_new, logx, logy, N_thread, True)
                lev.set_levin(n_sub, n_bisec_max, rel_acc, boost_bessel, verbose)
                
            ell_values = (val_ell*np.ones_like(k_int)).astype(int)
            lev.levin_integrate_bessel_single(lower_limit, upper_limit, k_int, ell_values, inner_int[idx_shell, i_ell,:])
            flat_idx += 1
            
            if load_levin is False:
                lev_list.append(lev)

    shell_cls = 2/np.pi*np.trapezoid(inner_int[:, None, :, :] * inner_int[None, : ,: ,:]*k_int**2,k_int, axis = -1)
